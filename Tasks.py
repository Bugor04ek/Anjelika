...
class WireDrawingTask(Task):
    ...
    def calculate_setup_time(current_task: "WireDrawingTask", previous_task: "WireDrawingTask"):
        ...
        def to_nested_tuple(obj):
            """
            Преобразует вложенные списки или numpy массивы в кортежи.
            """
            if isinstance(obj, (list, np.ndarray)):
                return tuple(to_nested_tuple(x) for x in obj)
            return obj  # Если это не список и не массив, возвращаем как есть
        ...
        if current_task.equipment.equipment_name != 'old':  # Новая машина
            for road1, road2 in zip(current_spin_road, previous_spin_road):
                ...
                if diff_indices.size == 0:
                    continue  # Волоки совпадают

                diff_spin = diff_indices[0]

                spin_out_values = road2[diff_spin:][road2[diff_spin:] != 0]
                spin_in_values = road1[diff_spin:][road1[diff_spin:] != 0]
                setup_time += calculate_spin_change(spin_out_values, spin_in_values)
            ...
            best_road = {}
            for road1 in current_spin_road:
                for road2 in previous_spin_road:
                    min_len = min(len(road1), len(road2))
                    diff_indices = np.where(road1[:min_len] != road2[:min_len])[0]
                    diff_spin = diff_indices[0] if diff_indices.size > 0 else len(road1)

                    spin_out_values = road2[diff_spin:][road2[diff_spin:] != 0]
                    spin_in_values = road1[diff_spin:][road1[diff_spin:] != 0]
                    temp_setup_time = calculate_spin_change(spin_out_values, spin_in_values)
                    current_task.comment_setup += f'снять {len(spin_out_values)} волок ({spin_out_values.tolist()}); '
                    current_task.comment_setup += f'вставить {len(spin_in_values)} волок ({spin_in_values.tolist()});'
...
class MultivareTask(Task):
    ...
    def calculate_setup_time(current_task: "MultivareTask", previous_task: "MultivareTask"):
        ...
        for road1 in current_task.spin_road:
            for road2 in previous_task.spin_road:
                # Преобразуем списки в NumPy массивы
                road1_array = np.array(road1)
                road2_array = np.array(road2)

                # Убираем нулевые значения из обоих массивов
                non_zero_road1 = road1_array[road1_array != 0]
                non_zero_road2 = road2_array[road2_array != 0]

                # Сравниваем только ненулевые значения
                min_length = min(len(non_zero_road1), len(non_zero_road2))
                diff_indices = np.where(non_zero_road1[:min_length] != non_zero_road2[:min_length])[0]

                if len(diff_indices) > 0:
                    # Снимаем ненулевые волоки из предыдущего задания
                    spin_out = len(non_zero_road2) - diff_indices[0]
                    setup_time += spin_out * MultivareMachine.CHANGE_WIRE + spin_out * MultivareMachine.REMOVED_SPIN

                    # Вставляем ненулевые волоки из текущего задания
                    spin_in = len(non_zero_road1) - diff_indices[0]
                    setup_time += spin_in * MultivareMachine.CHANGE_WIRE + spin_in * MultivareMachine.INSERT_SPIN

                    # Сохраняем результат для текущей пары маршрутов
                    best_road[(tuple(road1), tuple(road2))] = setup_time

        # Выбираем минимальное время перенастройки
        best_setup_time = min(best_road.values(), default=0)
        current_task.time_setup = best_setup_time
        return best_setup_time
