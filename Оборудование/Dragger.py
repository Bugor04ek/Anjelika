...
def calculate_setup_time_dragger(previous_order: TaskForDragger, order: TaskForDragger) -> float:
    ...
    if previous_order is not None:
        ...
        # меньше фильер -> больше диаметр
            ...
            total_setup_time += removed_spin * REMOVED_SPIN  # время на снятие фильер
            total_setup_time += INSERT_SPIN * (order.spin - (previous_order.spin - 1))  # время на установку фильер -1, потому что 1 уже снята
            change = True
        ...
        for orders in self.orders:
            ...
            if issubclass(Multivare.Basket, type(orders)):
                res += 'Корзина (Время работы заказов = {}ч. {}мин.; Длина {}): {}'.format(str(orders.time_work // 60),
                                                                                 str(round(orders.time_work % 60, 2)), orders.sum_basket,
                                                                                 orders.__str__())
