...
class Genetika:
    ...
    def __init__(self):
        ...
        self.matrix_setup_time: [[float]] = [[]]
        self.settings = None
        self.filters_array = None
        self.load_settings()
        self.get_filters()
        self.tasks = TaskMeta.get_instances_all()
        self.len_matrix = len(self.tasks)
        self.run_genetic_algorithm()

    def run_genetic_algorithm(self):
        # Получаем список задач
        Task.assign_tasks_to_equipment(self.tasks)
        self.matrix_setup_time = Task.form_matrix_setup_time(self.tasks, self.len_matrix)
        # константы задачи
        HALL_OF_FAME_SIZE = round((self.len_matrix * self.settings.get("HALL_OF_FAME_SIZE", 2)))  # Количество лучших индивидуумов
        POPULATION_SIZE = round(self.len_matrix * self.settings.get("POPULATION_SIZE", 1))  # количество индивидуумов в популяции
        MAX_GENERATIONS = round(self.len_matrix * self.settings.get("MAX_GENERATIONS", 10))  # Максимальное количество поколений
        P_CROSSOVER = self.settings.get("P_CROSSOVER", 0)  # Вероятность скрещивания
        P_MUTATION = self.settings.get("P_MUTATION", 0.05)  # Вероятность мутации
        MULTITHREADING = self.settings.get("MULTITHREADING", 1)  # Многопоточность
        ...
        print(f"REMAINING_BASKET_LENGTH: {self.settings.get('REMAINING_BASKET_LENGTH', 8)}")
        print(f"CORES: {self.settings.get('CORES', 2)}")
        print(f"LOGS: {self.settings.get('LOGS')}")
        ...
        population, logbook = eaSimpleWithElitism(
            ...
            stats=stats,
            halloffame=hof,
            verbose=self.settings.get('LOGS')
        )
        ...
        if self.settings.get('LOGS'):
            print(formatted_solution)

        Genetika.update_remaining_basket_length(8 - best_order.Basket[-1].sum_basket)
        return best_order
    ...
    def generate_individual(self):
        ...
        updated_baskets = []
        updated_baskets.extend(self.calculating_basket(individual))

        for i, basket in enumerate(updated_baskets):
            ind = individual[basket.equipment_type][basket.equipment.equipment_name]
            ind.insert(random.randint(0, len(ind)), self.len_matrix + 1)
    ...
    def calculating_basket(self, ind):
        """
        ...
        for eq in ind['multivare']:
            ...
            for order in task_m:
                self.settings.get("REMAINING_BASKET_LENGTH", TaskMeta.get_instances_by_type(index=order)[0].equipment.remaining_basket_length)
            # MultivareTask.calculate_setup_time_all(task_m)
            temp_basket: Basket = copy.deepcopy(Basket(None))
            for order in task_m:
                order = TaskMeta.get_instances_by_type(index=order)[0]
    ...
    def evaluate_fitness(self, individual: dict):
        ...
        multivare_time = self.get_cost_multivare(individual)
        drawing_time = self.get_cost_drawing(individual)
    ...
    def get_cost_drawing(self, ind, print_logs=False):
        ...
        filters_dict = {}
        for filter_item in self.filters_array:
            filter_item["ВремяОсвобождения"] = today_midnight
            filters_dict[filter_item['Диаметр']] = {"ВремяОкончанияРаботы": today_midnight,
    ...
    def get_cost_basket(self, tasks_with_basket, baskets):
        ...
        routes = self.get_routes(tasks_with_basket, baskets)
        ...
        time_routes = np.array([self.get_time_route(route) for route in routes])
    ...
    def get_time_route(self, tasks: [WireDrawingTask]):
        """
        ...
        for i in range(1, len(tasks)):
            time += self.matrix_setup_time[i][i - 1]
    ...
    def get_routes(self, tasks, baskets):
        """
...
def setup_time_begin_end(tasks):
    for i in range(len(tasks)):
        tasks[i].time_begin = (tasks[i - 1].time_ending +
                               timedelta(minutes=tasks[i].time_setup))  # Время начала заказа - время конца предыдущего заказа + время перенастройки
        tasks[i].time_ending = (tasks[i].time_begin +
                                timedelta(minutes=tasks[i].time_work))  # Время конца заказа - время начала текущего заказа + время в работе
    return tasks
